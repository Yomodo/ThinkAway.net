...
namespace ThinkAway.Drawing.Barcode.Common.detector
{
	...
	public sealed class MonochromeRectangleDetector
	{
		...
		/// the barcode
		/// </param>
		/// <returns> a {@link ThinkAway.Drawing.Barcode.ResultPoint} encapsulating the corner that was found
		/// </returns>
		/// <throws>  ThinkAway.Drawing.Barcode.ReaderException if such a point cannot be found </throws>
		private ResultPoint findCornerFromCenter(int centerX, int deltaX, int left, int right, int centerY, int deltaY, int top, int bottom, int maxWhiteRun)
		{
			...
			{
			    int[] range = deltaX == 0 ? blackWhiteRange(y, maxWhiteRun, left, right, true) : blackWhiteRange(x, maxWhiteRun, top, bottom, false);
				if (range == null)
				{
					if (lastRange == null)
					{
		...
		private int[] blackWhiteRange(int fixedDimension, int maxWhiteRun, int minDim, int maxDim, bool horizontal)
		{
			...
			while (start >= minDim)
			{
				if (horizontal?_image.get_Renamed(start, fixedDimension):_image.get_Renamed(fixedDimension, start))
				{
					start--;
				}
				else
				{
					...
					while (start >= minDim && !(horizontal?_image.get_Renamed(start, fixedDimension):_image.get_Renamed(fixedDimension, start)));
			...
			while (end < maxDim)
			{
				if (horizontal?_image.get_Renamed(end, fixedDimension):_image.get_Renamed(fixedDimension, end))
				{
					end++;
				}
				else
				{
					...
					while (end < maxDim && !(horizontal?_image.get_Renamed(end, fixedDimension):_image.get_Renamed(fixedDimension, end)));
