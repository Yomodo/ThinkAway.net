...
using System.IO;
using System.Text;
using ThinkAway.IO.ZipLib.Tar;

namespace ThinkAway.IO.ZipLib.Tar
{
	/// <summary>
		...
		void WriteEntryCore(TarEntry sourceEntry, bool recurse)
		{
			...
			if (this.asciiTranslate && !entry.IsDirectory) {
				...
				if (asciiTrans) {
					...
					using (StreamReader inStream  = System.IO.File.OpenText(entryFilename)) {
						using (Stream outStream = System.IO.File.Create(tempFileName)) {
			...
			if (entry.IsDirectory) {
				...
				using (Stream inputStream = System.IO.File.OpenRead(entryFilename)) {
					int numWritten = 0;
