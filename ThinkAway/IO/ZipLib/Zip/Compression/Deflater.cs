...
namespace ThinkAway.IO.ZipLib.Zip.Compression
{
	...
	public class Deflater
	{
		...
		*/
		...
		public void Reset()
		{
			state = (_noZlibHeaderOrFooter ? BUSY_STATE : INIT_STATE);
			totalOut = 0;
			_pending.Reset();
			_engine.Reset();
		}
		...
		public bool IsFinished {
			get {
				return (state == FINISHED_STATE) && _pending.IsFlushed;
			}
		}
		...
		public int Deflate(byte[] output, int offset, int length)
		{
			...
			for (;;) {
				...
				if (!_engine.Deflate((state & IS_FLUSHING) != 0, (state & IS_FINISHING) != 0))
				{
				    if (state == BUSY_STATE) {
