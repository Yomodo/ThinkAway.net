...
namespace ThinkAway.IO.ZipLib.Zip.Compression 
{
	...
	public class DeflaterEngine : DeflaterConstants 
	{
		...
		public void SetDictionary(byte[] buffer, int offset, int length) 
		{
			...
			System.Array.Copy(buffer, offset, _window, strstart, length);
				...
				switch (compressionFunction) {
					case DEFLATE_STORED:
						if (strstart > blockStart) {
							_huffman.FlushStoredBlock(_window, blockStart,
					...
					case DEFLATE_FAST:
						if (strstart > blockStart) {
							_huffman.FlushBlock(_window, blockStart, strstart - blockStart,
								false);
							blockStart = strstart;
						}
						break;
					...
					case DEFLATE_SLOW:
						if (prevAvailable) {
							_huffman.TallyLit(_window[strstart-1] & 0xff);
						}
						if (strstart > blockStart) {
							_huffman.FlushBlock(_window, blockStart, strstart - blockStart, false);
							blockStart = strstart;
						}
						prevAvailable = false;
						matchLen = MIN_MATCH - 1;
		...
		public void FillWindow()
		{
			...
			while (lookahead < DeflaterConstants.MIN_LOOKAHEAD && inputOff < inputEnd) 
			{
				...
				System.Array.Copy(inputBuf, inputOff, _window, strstart + lookahead, more);
				_adler.Update(inputBuf, inputOff, more);
...
*/
			ins_h = (_window[strstart] << HASH_SHIFT) ^ _window[strstart + 1];
		}
		...
		{
			short match;
			int hash = ((ins_h << HASH_SHIFT) ^ _window[strstart + (MIN_MATCH -1)]) & HASH_MASK;
...
#endif
			_prev[strstart & WMASK] = match = _head[hash];
			_head[hash] = unchecked((short)strstart);
			ins_h = hash;
			return match & 0xffff;
		}
		...
		bool FindLongestMatch(int curMatch) 
		{
			...
			byte scan_end1 = _window[best_end - 1];
			byte scan_end  = _window[best_end];
			...
			do {
...
#endif            
				if (_window[curMatch + best_len] != scan_end      || 
					_window[curMatch + best_len - 1] != scan_end1 || 
				...
				*/
				...
				if (scan > best_end) {
					...
					scan_end1  = _window[best_end - 1];
					scan_end   = _window[best_end];
				}
				scan = strstart;
		...
		bool DeflateStored(bool flush, bool finish)
		{
			...
			if ((storedLength >= DeflaterConstants.MAX_BLOCK_SIZE) || // Block is full
				...
				_huffman.FlushStoredBlock(_window, blockStart, storedLength, lastBlock);
		...
		bool DeflateFast(bool flush, bool finish)
		{
			...
			while (lookahead >= MIN_LOOKAHEAD || flush) {
				if (lookahead == 0) {
					...
					_huffman.FlushBlock(_window, blockStart, strstart - blockStart, finish);
					blockStart = strstart;
					return false;
				}
				...
				if (lookahead >= MIN_MATCH && 
					...
					bool full = _huffman.TallyDist(strstart - matchStart, matchLen);
					...
					// No match found
					_huffman.TallyLit(_window[strstart] & 0xff);
					++strstart;
					--lookahead;
				}
				...
				if (_huffman.IsFull()) {
					...
					_huffman.FlushBlock(_window, blockStart, strstart - blockStart, lastBlock);
					blockStart = strstart;
					return !lastBlock;
				}
			}
		...
		bool DeflateSlow(bool flush, bool finish)
		{
			...
			while (lookahead >= MIN_LOOKAHEAD || flush) {
...
#endif
					_huffman.TallyDist(strstart - 1 - prevMatch, prevLen);
					prevLen -= 2;
					do {
						strstart++;
						lookahead--;
				...
				if (_huffman.IsFull()) {
					...
					_huffman.FlushBlock(_window, blockStart, len, lastBlock);
					blockStart += len;
					return !lastBlock;
				}
			}
