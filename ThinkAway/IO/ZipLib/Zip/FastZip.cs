...
namespace ThinkAway.IO.ZipLib.Zip
{
	...
	public class FastZip
	{
		...
		void AddFileContents(string name, Stream stream)
		{
			...
			if( _events!=null ) {
				continueRunning_=_events.OnCompletedFile(name);
			}
		}
		...
		void ExtractFileEntry(ZipEntry entry, string targetName)
		{
			...
			if ( proceed ) {
				if ( _events != null ) {
					continueRunning_ = _events.OnProcessFile(entry.Name);
				}
				...
				if ( continueRunning_ ) {
					try {
						using ( FileStream outputStream = File.Create(targetName) ) {
							...
							if (_events != null) {
								continueRunning_ = _events.OnCompletedFile(entry.Name);
							}
						}
						...
						if ( _events != null ) {
							continueRunning_ = _events.OnFileFailure(targetName, ex);
						}
						else {
			...
			if ( doExtraction && !Directory.Exists(dirName) ) {
				if ( !entry.IsDirectory || CreateEmptyDirectories ) {
					try {
						...
						doExtraction = false;
						if ( _events != null ) {
							continueRunning_ = entry.IsDirectory ? _events.OnDirectoryFailure(targetName, ex) : _events.OnFileFailure(targetName, ex);
						}
						else {
							continueRunning_ = false;
                            throw;
	    ...
	    readonly FastZipEvents _events;
