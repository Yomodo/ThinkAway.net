...
namespace ThinkAway.IO.ZipLib.Zip
{
	...
	public class ZipEntry : ICloneable
	{
		...
		{
			get {
				return ( _flags & (int)GeneralBitFlags.UnicodeText ) != 0;
			}
			set {
				if ( value ) {
	    ...
	    private byte _cryptoCheckValue;
	    ...
	    {
	        get { return _cryptoCheckValue; }
	        set { _cryptoCheckValue = value; }
	    }
		...
		public bool LocalHeaderRequiresZip64 
		{
			get {
				...
				if ( !result ) {
					...
					result =
						((this._size >= uint.MaxValue) || (trueCompressedSize >= uint.MaxValue)) &&
		...
		{
			get {
				return LocalHeaderRequiresZip64 || (_offset >= uint.MaxValue);
			}
		}
		...
		public DateTime DateTime 
		{
			get {
				...
				int day = Math.Max(1, Math.Min(DateTime.DaysInMonth((int)year, (int)mon), (int)((_dosTime >> 16) & 0x1f)));
		...
		{
			get {
				return (known & Known.Size) != 0 ? (long)_size : -1L;
			}
			set {
		...
		{
			get {
				return (known & Known.CompressedSize) != 0 ? (long)_compressedSize : -1L;
			}
			set {
		...
		{
			get {
				return (known & Known.Crc) != 0 ? _crc & 0xffffffffL : -1L;
			}
			set {
...
//				return (byte[]) extra.Clone();
		...
		{
			ZipExtraData extraData = new ZipExtraData(this._extra);

			if ( extraData.Find(0x0001) ) {
				if ( (_versionToExtract & 0xff) < ZipConstants.VersionZip64 ) {
		...
		ulong  _size;
		ulong  _compressedSize;
	    readonly ushort _versionToExtract;                // Version required to extract (library handles <= 2.0)
		uint   _crc;
		uint   _dosTime;
		...
		int _flags;                             // general purpose bit flags

		long _zipFileIndex = -1;                // used by ZipFile
		long _offset;                           // used by ZipFile and ZipOutputStream
