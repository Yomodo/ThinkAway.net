...
using ThinkAway.IO.ZipLib.Encryption;
using ThinkAway.IO.ZipLib.Core;
using ThinkAway.IO.ZipLib.Checksums;
using ThinkAway.IO.ZipLib.Zip.Compression;
using ThinkAway.IO.ZipLib.Zip.Compression.Streams;

#if !NETCF_1_0
#endif

namespace ThinkAway.IO.ZipLib.Zip 
{
	...
	public class TestStatus
	{
	    ...
	    readonly ZipFile _file;
		ZipEntry _entry;
		bool _entryValid;
		int _errorCount;
		long _bytesTested;
		TestOperation _operation;
		#endregion
	}
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		{
			set {
			    Key = string.IsNullOrEmpty(value) ? null : PkzipClassic.GenerateKeys(ZipConstants.ConvertToArray(value));
			}
		}
#endif
		...
		public ZipFile(string name)
		{
			...
			_baseStream = File.OpenRead(name);
			isStreamOwner = true;
		...
		public ZipEntry GetEntry(string name)
		{
			...
			return (index >= 0) ? (ZipEntry) _entries[index].Clone() : null;
		}
		...
		public Stream GetInputStream(long entryIndex)
		{
			...
			Stream result = new PartialInputStream(this, start, _entries[entryIndex].CompressedSize);

			if (_entries[entryIndex].IsCrypted) {
#if NETCF_1_0
				...
				while ( testing && (entryIndex < Count) ) {
					...
					if ( testing && testData && this[entryIndex].IsFile ) {
						...
						if (( this[entryIndex].Flags & (int)GeneralBitFlags.Descriptor) != 0 ) {
							ZipHelperStream helper = new ZipHelperStream(_baseStream);
			...
			try {
				...
				if( _contentsEdited ) {
					...
					if( _entries.Length==0 ) {
						byte[] theComment=(_newComment!=null)?_newComment.RawComment:ZipConstants.ConvertToArray(_comment);
...
*/
		...
		int WriteCentralDirectoryHeader(ZipEntry entry)
		{
			...
			if ( entry.ExternalFileAttributes != -1 ) {
			    ...
			    WriteLEUint((uint) (entry.IsDirectory ? 16 : 0));
			}
		...
		protected string GetTransformedDirectoryName(string name)
		{
			return (NameTransform != null) ?
		...
		byte[] GetBuffer()
		{
		    return _copyBuffer ?? (_copyBuffer = new byte[_bufferSize]);
		}
		...
		int FindExistingUpdate(ZipEntry entry)
		{
			...
			if (_updateIndex.ContainsKey(convertedName)) {
				result = (int)_updateIndex[convertedName];
			}
/*
 ...
 */
		...
		void AddEntry(ZipFile workFile, ZipUpdate update)
		{
			...
			if ( update.Entry.IsFile ) {
				source = update.GetSource() ?? _updateDataSource.GetSource(update.Entry, update.Filename);
			}
			...
			if ( source != null ) {
				using ( source ) {
					...
					{
						ZipHelperStream helper = new ZipHelperStream(workFile._baseStream);
		...
		void RunUpdates()
		{
			...
			if ( IsNewArchive ) {
				...
				workFile = ZipFile.Create(_archiveStorage.GetTemporaryOutput());
			...
			try {
			    ...
			    foreach (ZipUpdate update in _updates)
			    {
			        if (update != null) sizeEntries = sizeEntries + workFile.WriteCentralDirectoryHeader(update.OutEntry);
			    }
		...
		class ZipUpdate
		{
			...
			*/
			...
			{
				get { return _outEntry ?? (_outEntry = (ZipEntry) _entry.Clone()); }
			}
			...
			{
				get { return _sizePatchOffset; }
				set { _sizePatchOffset = value; }
			}
			...
			{
				get { return _crcPatchOffset; }
				set { _crcPatchOffset = value; }
			}
			...
			ZipEntry _outEntry;
		    readonly UpdateCommand _command;
		    readonly IStaticDataSource _dataSource;
		    readonly string FileName;
			long _sizePatchOffset = -1;
			long _crcPatchOffset = -1;
			#endregion
		}
		...
		ArrayList _updates;
		long _updateCount; // Count is managed manually as updates_ can contain nulls!
		Hashtable _updateIndex;
		IArchiveStorage _archiveStorage;
		IDynamicDataSource _updateDataSource;
		bool _contentsEdited;
		int _bufferSize = DefaultBufferSize;
		byte[] _copyBuffer;
		ZipString _newComment;
		bool _commentEdited;
		IEntryFactory _updateEntryFactory = new ZipEntryFactory();
		#endregion
		#endregion
		...
		class PartialInputStream : Stream
		{
			...
			public PartialInputStream(ZipFile zipFile, long start, long length)
			{
				...
				// uses reader here....
				_zipFile = zipFile;
				_baseStream = _zipFile._baseStream;
				_readPos = start;
				_end = start + length;
			}
			#endregion
	...
	abstract public class BaseArchiveStorage : IArchiveStorage
	{
	    ...
	    readonly FileUpdateMode _updateMode;
		#endregion
	}
	...
	public class DiskArchiveStorage : BaseArchiveStorage
	{
		...
		public override Stream GetTemporaryOutput()
		{
			if ( _temporaryName != null ) {
				...
				_temporaryStream = File.OpenWrite(_temporaryName);
			}
			else {
		...
		public override Stream ConvertTemporaryToFinal()
		{
			...
			try	{
				...
				File.Move(_temporaryName, FileName);
				newFileCreated = true;
		...
		public override Stream MakeTemporaryCopy(Stream stream)
		{
			...
			_temporaryName = GetTempFileName(FileName, true);
			File.Copy(FileName, _temporaryName, true);
			
			_temporaryStream = new FileStream(_temporaryName, 
				FileMode.Open, 
	    ...
	    /// <param name="stream">The current stream.</param>
	    /// <returns>Returns a stream suitable for direct updating.</returns>
	    /// <remarks>If the <paramref>
	    ///                   <name>current</name>
	    ///                 </paramref>
	    ///   stream is not null this is used as is.</remarks>
	    public override Stream OpenForDirectUpdate(Stream stream)
		{
			...
			if ((stream == null) || !stream.CanWrite)
			{
				...
				result = new FileStream(FileName,
						FileMode.Open,
						FileAccess.ReadWrite);
			}
			else
