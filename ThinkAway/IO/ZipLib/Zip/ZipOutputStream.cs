...
using Crc32 = ThinkAway.IO.Disc.Crc32;
...
namespace ThinkAway.IO.ZipLib.Zip
{
	...
	public class ZipOutputStream : DeflaterOutputStream
	{
		...
		public void PutNextEntry(ZipEntry entry)
		{
			...
			if ( (_useZip64 == UseZip64.On) || ((entry.Size < 0) && (_useZip64 == UseZip64.Dynamic)) ) {
				entry.ForceZip64();
			}
			...
			if (entry.LocalHeaderRequiresZip64) {
				...
				if ( _patchEntryHeader ) {
					_sizePatchPos = ed.CurrentReadIndex;
				}
			}
			else {
				ed.Delete(1);
		...
		public void CloseEntry()
		{
			...
			if (_curEntry.Size < 0) {
				...
				throw new ZipException("size was " + _size + ", but I expected " + _curEntry.Size);
			}
			
			if (_curEntry.CompressedSize < 0) {
				_curEntry.CompressedSize = csize;
			} else if (_curEntry.CompressedSize != csize) {
				throw new ZipException("compressed size was " + csize + ", but I expected " + _curEntry.CompressedSize);
			}
			
			if (_curEntry.Crc < 0) {
				_curEntry.Crc = _crc.Value;
			} else if (_curEntry.Crc != _crc.Value) {
				throw new ZipException("crc was " + _crc.Value +	", but I expected " + _curEntry.Crc);
			}
			
			_offset += csize;

			if (_curEntry.IsCrypted) {
				_curEntry.CompressedSize += ZipConstants.CryptoHeaderSize;
			}
			...
			if ((_curEntry.Flags & 8) != 0) {
				...
				WriteLeInt(unchecked((int)_curEntry.Crc));
		...
		public override void Finish()
		{
			...
			long numEntries = _entries.Count;
			long sizeEntries = 0;
			...
			using ( ZipHelperStream zhs = new ZipHelperStream(baseOutputStream_) ) {
				zhs.WriteEndOfCentralDirectory(numEntries, sizeEntries, _offset, _zipComment);
			}
		...
		/// </summary>
		ArrayList _entries  = new ArrayList();
		
		/// <summary>
		/// Used to track the crc of data added to entries.
		/// </summary>
        readonly Crc32Checksum _crc = new Crc32Checksum();
		...
		int _defaultCompressionLevel = Deflater.DEFAULT_COMPRESSION;
		
		CompressionMethod _curMethod = CompressionMethod.Deflated;
