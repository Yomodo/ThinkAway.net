...
namespace ThinkAway.Net.Mail.IMAP
{
	/// <summary>
	/// 
	/// </summary>
	public class ImapBase
	{
		...
		protected string IMAP_COMMAND_IDENTIFIER 
		{
			get
			{ 
				return string.Format("{0}{1} ", IMAP_COMMAND_PREFIX, IMAP_COMMAND_VAL);
			}
		...
		protected ImapResponseEnum Connect(string sHost, ushort nPort) 
		{
			...
			try 
			{
				...
				if (sResult.StartsWith(IMAP_OK_SERVER_RESPONSE))
				{
			...
			{
                throw new ImapException(ImapException.ImapErrorEnum.IMAP_ERR_CONNECT, sHost); ;
			}
			m_sHost = sHost;	
			m_nPort = nPort;  
		...
		public void Capability()
		{
		    ...
		    ImapResponseEnum eImapResponse = SendAndReceive(capabilityCommand, ref sResultArray);
