...
namespace ThinkAway.Net.Mail.POP3
{
	...
	internal static class CramMd5
	{
		...
		internal static string ComputeDigest(string username, string password, string challenge)
		{
			...
			return Convert.ToBase64String(Encoding.ASCII.GetBytes(string.Format("{0} {1}", username, hex)));
		}
		...
		private static byte[] Concatenate(byte[] one, byte[] two)
		{
			...
			System.Buffer.BlockCopy(one, 0, concatenated, 0, one.Length);

			// Copy the second one over
			System.Buffer.BlockCopy(two, 0, concatenated, one.Length, two.Length);
