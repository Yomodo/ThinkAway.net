...
namespace ThinkAway.Net.Mail.POP3
{
	...
	public class MessagePart
	{
		...
		public Encoding BodyEncoding
	    {
	        ...
	        private set { _bodyEncoding = value; }
	    }
		...
		public List<MessagePart> MessageParts
	    {
	        ...
	        private set { _messageParts = value; }
	    }
		...
		private static List<byte[]> GetMultiPartParts(byte[] rawBody, string multipPartBoundary)
		{
			...
			using (MemoryStream stream = new MemoryStream(rawBody))
			{
				...
				while (true)
				{
					...
					startLocation = stopLocation + ("\r\n--" + multipPartBoundary + Environment.NewLine).Length;
				}
			}
		...
		private static int FindPositionOfNextMultiPartBoundary(Stream stream, string multipPartBoundary)
		{
			while(true)
			{
				...
				// Read the line
                string line = FileHelper.ReadLineAsAscii(stream);
