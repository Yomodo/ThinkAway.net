...
namespace ThinkAway.Net.Mail.POP3
{
    /// <summary>
    ...
    public class PopClient : Disposable
    {
        ...
        /// the <see cref="AuthMethod.Apop"/> authentication method.<br/>
        /// <br/>
        ...
        {
            get { return _apopSupported; }
            private set { _apopSupported = value; }
        }
        ...
        public PopClient(string host) : this(host, 110)
        {
        }
        ...
        /// <summary>
        /// Disposes the <see cref="PopClient"/>.<br/>
        ...
        protected void Connect(Stream stream)
        {
            AssertDisposed();
            ...
            string response = FileHelper.ReadLineAsAscii(Stream);
            ...
            catch (PopServerException e)
            {
                ...
                //System.Console.WriteLine("Last response from server was: {0}", LastServerResponse);
                throw new PopServerNotAvailableException("Server is not available: Connect(): Error with connection, maybe POP3 server not exist", e);
            }
        }
        ...
        /// </summary>
        /// <param name="host">The <paramref name="host"/> of the POP3 server</param>
        ...
        public void Connect(string host, int port, bool useSsl)
        {
            ...
            Connect(host, port, useSsl, defaultTimeOut, defaultTimeOut, null);
        }
        ...
        public void Connect(string host, int port, bool useSsl, int receiveTimeout, int sendTimeout, RemoteCertificateValidationCallback certificateValidator)
        {
            AssertDisposed();
            ...
            catch (SocketException e)
            {
                ...
                System.Console.WriteLine("Connect(): {0}", e.Message);
            ...
            if (useSsl)
            {
                ...
                SslStream sslStream = certificateValidator == null
        ...
        /// <summary>
        /// Authenticates a user towards the POP server using <see cref="AuthMethod.Auto"/>.
        /// </summary>
        ...
        {
            AssertDisposed();
            Authenticate(username, password, AuthMethod.Auto);
        }
        ...
        private void AuthenticateUsingUserAndPassword(string username, string password)
        {
            ...
            {
                throw new InvalidLoginException(string.Format("AuthenticateUsingUserAndPassword():wrong user: {0}", username), e);
            }
            ...
            catch (PopServerException e)
            {
                ...
                //System.Console.WriteLine("Server response was: " + LastServerResponse);
                throw new InvalidLoginException("AuthenticateUsingUserAndPassword(): wrong password.", e);
            }
        }
        ...
        private void AuthenticateUsingApop(string username, string password)
        {
            ...
            try
            {
                SendCommand(string.Format("APOP {0} {1}", username, Apop.ComputeDigest(password, ApopTimeStamp)));
            }
            catch (PopServerException e)
            {
                CheckFailedLoginServerResponse(LastServerResponse, e);
                ...
                //System.Console.WriteLine("Server response was: " + LastServerResponse);
                throw new InvalidLoginException("AuthenticateUsingApop(): wrong user or password", e);
            }
        }
        ...
        private void AuthenticateUsingCramMd5(string username, string password)
        {
            ...
            catch (PopServerException e)
            {
                ...
                //System.Console.WriteLine("Server response was: " + LastServerResponse);
                throw new InvalidLoginException("AuthenticateUsingCramMd5(): wrong user or password", e);
            }
        ...
        public List<string> GetMessageUids()
        {
            ...
            while (!IsLastLineInMultiLineResponse(response = FileHelper.ReadLineAsAscii(Stream)))
            {
        ...
        public List<int> GetMessageSizes()
        {
            ...
            while (!".".Equals(response = FileHelper.ReadLineAsAscii(Stream)))
            {
        ...
        public MailMessage GetMessage(int messageNumber)
        {
            ...
            return new MailMessage(messageContent);
        }
        ...
        public MessageHeader GetMessageHeaders(int messageNumber)
        {
            ...
            return new MailMessage(messageContent, false).Headers;
        }
        ...
        public Dictionary<string, List<string>> Capabilities()
        {
            ...
            while (!IsLastLineInMultiLineResponse(lineRead = FileHelper.ReadLineAsAscii(Stream)))
            {
        ...
        private static void IsOkResponse(string response)
        {
            ...
            throw new PopServerException(string.Format("The server did not respond with a + response. The response was: \"{0}\"", response));
        }
        ...
        private void SendCommand(string command)
        {
            ...
            LastServerResponse = FileHelper.ReadLineAsAscii(Stream);
        ...
        private byte[] GetMessageAsBytes(int messageNumber, bool askOnlyForHeaders)
        {
            ...
            using (MemoryStream byteArrayBuilder = new MemoryStream())
            {
                ...
                // Keep reading until we are at the end of the multi line response
                while (!IsLastLineInMultiLineResponse(lineRead = FileHelper.ReadLineAsBytes(Stream)))
                {
                    ...
                    bool startingWithPeriod = lineRead.Length > 0 && Equals(lineRead[0], '.');
        ...
        private static void ValidateMessageNumber(int messageNumber)
        {
            if (messageNumber <= 0)
            {
                ArgumentException argumentException = new ArgumentException("The messageNumber argument cannot have a value of zero or less. Valid messageNumber is in the range [1, messageCount]", "messageNumber");
                throw new InvalidUseException("The messageNumber argument cannot have a value of zero or less. Valid messageNumber is in the range [1, messageCount]", argumentException);
            }
