...
namespace ThinkAway.Text.MIME.Decode
{
	/// <summary>
	...
	internal static class Rfc2231Decoder
	{
		...
		{
			string[] splitted = toDecode.Trim().Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
			...
			foreach (string part in splitted)
			{
				...
				{
					throw new ArgumentException(string.Format("When splitting the part \"{0}\" by = there was {1} parts. Only 1 and 2 are supported", part, keyValue.Length));
				}
			}
		...
		private static List<KeyValuePair<string, string>> DecodePairs(List<KeyValuePair<string, string>> pairs)
		{
			...
			for(int i = 0; i<pairsCount; i++)
			{
                ...
                string value = StringHelper.RemoveQuotesIfAny(currentPair.Value);
				...
				if(key.EndsWith("*0", StringComparison.OrdinalIgnoreCase) || key.EndsWith("*0*", StringComparison.OrdinalIgnoreCase))
				{
					...
					for (int j = i + 1, continuationCount = 1; j < pairsCount; j++, continuationCount++)
					{
                        ...
                        string valueJKey = StringHelper.RemoveQuotesIfAny(pairs[j].Value);
						...
						else if (jKey.Equals(string.Format("{0}*{1}*", key, continuationCount)))
						{
		...
		private static string DecodeSingleValue(string valueToDecode, string encoding)
		{
			...
			valueToDecode = string.Format("=?{0}?Q?{1}?=", encoding, valueToDecode.Replace("%", "="));
