...
namespace ThinkAway.Text.MIME
{
	...
	public class TnefMessage {
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="TnefMessage" /> class based on the supplied <see cref="System.IO.Stream" />.
		/// </summary>
		...
		public TnefMessage ( System.IO.Stream input ) {
		...
		public bool Parse ( System.String path ) {
			...
			System.Text.Encoding enc = MimeHeader.EncodingDefault;
			SharpAttachment attachment_cur = null; 
			for ( System.Byte cur=this.ReadByte(); cur!=System.Byte.MinValue; cur=ReadByte() ) {
				TnefLvlType lvl = (TnefLvlType)ThinkAway.Text.MIME.MimeTools.ParseEnum(typeof(TnefLvlType), cur, TnefLvlType.Unknown);
				// Type
				int type = this.ReadInt();
				// Size
				int size = this.ReadInt();
				// Attibute name and type
				TnefAttribute att_n = (TnefAttribute)ThinkAway.Text.MIME.MimeTools.ParseEnum(typeof(TnefAttribute), (ushort)((type<<16)>>16), TnefAttribute.Unknown);
				TnefDataType att_t = (TnefDataType)ThinkAway.Text.MIME.MimeTools.ParseEnum(typeof(TnefDataType), (ushort)(type>>16), TnefDataType.Unknown);
				...
				if ( lvl==TnefLvlType.Message ) {
					...
					if ( att_n==TnefAttribute.AttachRendData ) {
						...
						attachment_cur = path==null ? new SharpAttachment(new System.IO.MemoryStream()) : new SharpAttachment(new System.IO.FileInfo(System.IO.Path.Combine(path, name)));
						...
						if ( attachment_cur!=null && att_t==TnefDataType.atpByte && buffer!=null ) {
							if ( attachment_cur.SavedFile!=null ) {
								...
								try {
                                    ...
                                    System.Console.WriteLine("OpenWrite:{0}",exception);
									error = true;
									break;
								}
